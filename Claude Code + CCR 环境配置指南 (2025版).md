...
## 这是一份为您精心整理的 Claude Code + CCR (Claude Code Router) 环境配置指南。它涵盖了您今晚探索出的所有“坑点”与解决方案，特别是针对国内网络环境和多模型切换的优化。

您可以直接拷贝以下 Markdown 源码保存为 CCR_Setup_Guide.md。
Markdown
...
在 CCR 的安装目录或通过 ccr config 命令编辑配置文件。
推荐配置模板：
JSON
...
启动 Claude Code: 首次运行需指定本地 API Base：
PowerShell
